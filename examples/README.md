...
|---|---|---|
| [`minimal`](minimal) | the smallest loop: build a Runner, run one agent, read the result and trace | yes |
| [`delegation`](delegation) | a dynamic region where the entry agent delegates to a peer, plus the trace, driven by a scripted model via `ModelOptions.Client` | yes |
