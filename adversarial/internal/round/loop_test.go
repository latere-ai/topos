...
func TestEngineSingleForkSteadyState(t *testing.T) {
	...
	e := &Engine{
		...
		MaxRounds: 5, CostCap: 100000, TaskContext: "task", DiffPatch: "diff",
	}
	...
	if len(sum.Forks) != 1 || sum.Forks[0].Termination != TermSteadyState {
		t.Fatalf("fork termination at final round: got %+v, want steady-state", sum.Forks)
	}
...
func TestEngineRunSummaryReportsMaxTurn(t *testing.T) {
	sess, err := state.NewSession(t.TempDir(), 1, time.Now())
	if err != nil {
		t.Fatal(err)
	}
	...
	e := &Engine{
		...
		Proposer: &stubProposer{
			...
			next: func(string) (*agent.ProposerResult, error) {
				return &agent.ProposerResult{ForkID: "f", Response: "rebut: ok", Tokens: 10}, nil
			},
		},
		NewCritic:   func(_ int) agent.Critic { return &stubCritic{rounds: []string{r1, r3, r5}} },
		MaxRounds:   6,
	...
	if len(sum.Forks) != 1 || sum.Forks[0].Termination != TermMaxTurn {
		t.Fatalf("fork termination: got %+v, want max-turn", sum.Forks)
	}
	if sum.Termination != TermMaxTurn {
		t.Fatalf("run termination: got %s, want max-turn", sum.Termination)
	}
}
